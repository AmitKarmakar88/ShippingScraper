...
Destination=os.path.abspath(os.getcwd()).replace('LGL_complete','')
...
DelDateColIndex1=list(df.index[df['Date of Arrival (ETA)']==""]) 
#print(df.columns)
...
df.drop(DelDateColIndex1 , inplace=True)
...
for index,row in df.iterrows():
	...
	if type(row['Port Name'])!=type(np.nan):
		if row['Port Name'].lower()=='philadelphia':
			df['port_code'][index]="US PHL"
		
		elif row['Port Name'].lower()=='jacksonville':
			df['port_code'][index]="US JAX"

		elif row['Port Name'].lower()=='freeport':
			df['port_code'][index]="BS FPO"
			
			
		elif row['Port Name'].lower().strip()=='ulsan':
			df['port_code'][index]="KR USN"
			
			
		elif row['Port Name'].lower()=='charleston':
			df['port_code'][index]="US CHS"
			
			
		elif row['Port Name'].lower()=='inchon':
			df['port_code'][index]="KR INC"
			
			
			
		elif row['Port Name'].lower()=='karachi':
			df['port_code'][index]="PK KHI"
			
			
		elif row['Port Name'].lower()=='wilmington':
			df['port_code'][index]="US ILM"
			
		elif row['Port Name'].lower()=='uslan':
			df['port_code'][index]=" "
			
		elif row['Port Name'].lower()=='busan':
			df['port_code'][index]="KR PUS"
						   
		elif row['Port Name'].lower()=='higashi':
			df['port_code'][index]="JP HHR"
			
		elif row['Port Name'].lower()=='pusan':
			df['port_code'][index]="KR PUS"


		elif row['Port Name'].lower()=='pyongtaek':
			df['port_code'][index]=" "
					
		elif row['Port Name'].lower()=='hamad':
			df['port_code'][index]=" "

		elif row['Port Name'].lower()=='beaumont':
			df['port_code'][index]="US BPT"
		elif row['Port Name'].lower()=='baltimore':
...
df.to_csv("Complete_LGL.csv",index=False)

shutil.move("Complete_LGL.csv",Destination+"Complete_LGL.csv")
print(" LGL_complete")
